 ...
 */
public class DiabetesTest extends Test{

    public static void prepareData() throws IOException {
        dataSet = Files.readAllLines(Paths.get("/Users/sunweipeng/development/source/KMeans/src/Diabetes.data")).stream().map(new Function<String, PointWithLabel>() {
            @Override
    ...
    public static void main(String[] args) throws IOException {
       ...
       // KMeansModel model = KMeans.train(dataSet, 3, 4, 2, 1000);
        ...
        int d = 0;
        for(int desenity=1;desenity<400;desenity++) {
            model = new KMeans().setDataSet(dataSet).setK(3).setDim(8).setDesenity(desenity).setMaxIterations(1000).run();
