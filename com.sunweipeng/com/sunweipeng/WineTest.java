 ...
 */
public class WineTest extends Test{
    ...
    public static void main(String[] args) throws IOException {
        ...
        int desenity = 0;
        //初始化一个Kmean对象，将k置为10
        KMeansModel model = KMeans.train(dataSet, 3, 13, 10, 1000);
        ...
        System.out.println(1-1.0*errorcount/dataSet.size());
    }
}*/
        prepareData();
        preProcess(13);
        ...
        int d = 0;
        //初始化一个Kmean对象，将k置为10
        for(int desenity=1;desenity<100;desenity++) {
            model = new KMeans().setDataSet(dataSet).setK(3).setDim(13).setDesenity(desenity).setMaxIterations(1000).run();
            List<List<PointWithLabel>> clusters = model.getClusters();
            int errorcount = 0;
            for (int i = 0; i < clusters.size(); i++) {
