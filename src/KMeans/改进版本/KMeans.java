...
public class KMeans {
    ...
    private double getAvarageDistance() {
        ...
        for(int i=0;i<dataSet.size();i++) {
            for(int j=i+1;j<dataSet.size();j++) {
                sumDistance += distance(dataSet.get(i),dataSet.get(j));
            }
        }
    ...
    private double getAvarageRadius() {
        ...
        for(int i=0;i<dataSet.size();i++) {
            sumDistance += distance(dataSet.get(i),center);
        }
    ...
    private List<double[]> initCenters(int d) {
        ...
        for(int i=0;i<dataSet.size();i++) {
            ...
            for(int j=0;j<dataSet.size();j++) {
                if(i!=j){
                    distance.add(distance(dataSet.get(i),dataSet.get(j)));
                }
            }
        ...
        for(int i=0;i<candidates.size();i++) {
            for(int j=i+1;j<candidates.size();j++) {
                if(distance(dataSet.get(i),dataSet.get(j))<(candidates.get(i).radius+candidates.get(j).radius)){
        ...
        while(sets.size()<k){
            for(int i=0;i<sets.size();i++) {
                for(Candidate candidate:sets.get(i)) {
                    double temp = distance(centers.get(i),dataSet.get(candidate.index));
                    if(temp>max) {
                        max = temp;
                        isolate = candidate;
                        setIndex = i;
            ...
            for(Candidate candidate:splitSet) {
                if(distance(dataSet.get(isolate.index),dataSet.get(candidate.index))<distance(dataSet.get(candidate.index),centers.get(setIndex))){
    ...
    private List<double[]> calculateCenters(List<Set<Candidate>> sets) {
        ...
        for(int i=0;i<sets.size();i++) {
            ...
            for(int j=0;j<4;j++) {
                for(Candidate candidate: sets.get(i)) {
                    c[j]+=dataSet.get(candidate.index)[j]*candidate.radius/caudisSum;
                }
            }
            centers.add(c);
        }
