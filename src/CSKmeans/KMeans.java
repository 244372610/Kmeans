...
public class KMeans {
    ...
    private List<double[]> initCenters(int d) {
        ...
        List<Set<Candidate>> sets = getCollections(candidates);
    ...
    private Set<Candidate> getCollect(Candidate candidate,List<Candidate> candidates){
        Set<Candidate> cs = new HashSet<>();
        cs.add(candidate);
        candidates.remove(candidate);
        for(Candidate c:candidates) {
            if(distance(dataSet.get(candidate.index).getData(),dataSet.get(c.index).getData())<(candidates.get(candidate.index).radius+candidates.get(c.index).radius)){
                cs.addAll(getCollect(c,candidates));
            }
        }
        return cs;
    }

    private List<Set<Candidate>> getCollections(List<Candidate> candidates) {
        //开始合并
        List<Set<Candidate>> sets = new ArrayList<>();
        Iterator<Candidate> iterator = candidates.iterator();
        while(iterator.hasNext()){
            Candidate candidate = iterator.next();
            sets.add(getCollect(candidate,candidates));
        }
